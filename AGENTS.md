...
- Relevant completed follow-ups: annual maintenance norm import and hidden-row handling are complete on `to`; the first menu-rework iteration is committed/pushed on `to` as `8dfffbd Rework menu structure and safety prompts`; the `Net` branch network passport manual-review ergonomics package is accepted, committed, and pushed as `207b6b1 Improve network passport review ergonomics`; the follow-up `Net` manual-entry hints package is accepted, committed, and pushed as `a89e593 Improve network passport manual entry hints`.
- Current accepted `Net` baseline after the 2026-05-20 handoff is `a89e593` on `HEAD` and `origin/Net`. Current local `Net` work may include an uncommitted mutation-service validation package; check `docs/codex-handoff.md` before starting new work. Current UI/UX polishing continues separately in `C:\Users\Olga\AKB5-design` on `design/network-ui-polish`.
