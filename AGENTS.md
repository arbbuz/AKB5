...
- Current accepted `Net` baseline is `b2ea12e` on `C:\Users\Olga\AKB5` / `origin/Net`; recheck the main worktree before using it because local Net-side edits may exist. Current UI/UX polishing continues in `C:\Users\Olga\AKB5-design` on `design/network-ui-polish`; after `a2da5aa Polish network UI layout and splitter`, the design package adds the fixed 24px status bar and the approved style-only right-panel shell from `artifacts\previews\right-panel-style-only-components-preview.html`.
