...
- `Net` branch follow-up (2026-05-19) - network passport CRUD for devices, interfaces, and connections; manual scheme-entry fields; editable protocol/medium presets; passport filtering; robust network dialog layouts; PDF network scheme references as metadata/`Open original` sources; source-note comments for scheme references; device passport columns for vendor/location; interface passport columns for stored `MPI/DP/PN`, medium, speed, and notes; validation for manual interface IP/mask/gateway entry; validation for manually entered connection length; richer connection endpoint/length display; and copy-friendly device/interface/connection rows/endpoints plus visible filtered table export for manual scheme checks. OCR/PDF auto-import, PRONETA/CSV, live scan, plan/fact comparison, data-quality issue panels, and AKB5-driven IP/PROFINET assignment remain intentionally out of scope.
