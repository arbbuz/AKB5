    ...
    public class KnowledgeBaseCompositionMutationService
    {
            ...
            int visibleLevel = 0)
        {
            ...
            var normalizedDraft = new KbCompositionEntry
            {
                ...
                Model = model,
                OrderNumber = draftEntry.OrderNumber?.Trim() ?? string.Empty,
                Firmware = draftEntry.Firmware?.Trim() ?? string.Empty,
                MpiDpPnAddress = draftEntry.MpiDpPnAddress?.Trim() ?? string.Empty,
                InputAddress = draftEntry.InputAddress?.Trim() ?? string.Empty,
                OutputAddress = draftEntry.OutputAddress?.Trim() ?? string.Empty,
                Comment = draftEntry.Comment?.Trim() ?? string.Empty,
                InterfaceRows = draftEntry.InterfaceRows?.Trim() ?? string.Empty,
