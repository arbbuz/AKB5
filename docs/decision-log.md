...
- Startup optimization decision: keep the existing single-file publish as the default supported `scripts\publish.ps1` mode, and add a separate accepted fast working package via `scripts\publish-fast.ps1` that publishes a self-contained folder with ReadyToRun and no single-file extraction. Startup phase diagnostics should go to existing file logs as `StartupTiming` events.
- Network topology viewport decision: keep topology coordinates logical and resolution-independent; solve different monitor sizes with a scrollable viewport and `Ctrl + mouse wheel` zoom instead of rewriting saved element positions. Node labels must be drawn through the same scaled graphics pipeline as cards/icons. The link-kind selector stays visible as a fixed bottom strip below the viewport while the diagram scrolls/zooms above it.
- Network topology link-type selector decision: remove the long toolbar ComboBox and use a bottom strip below the topology viewport instead of a floating canvas overlay. Use full choices (`Profibus, оптоволокно`, `Profibus, медь`, `MPI, медь`, `Profinet, оптоволокно`, `Profinet, медь`) plus the real line color/dash sample. Button widths should be measured from the full regular/bold text with a safety margin rather than fixed short widths so labels are not clipped or ellipsized; on narrow resolutions the strip may scroll horizontally instead of cutting button text. Clicking a link selects it, and strip clicks edit the selected link immediately; otherwise the strip sets the kind for the next new link.
...
- UI/UX polish for the current review-filter/layout package is isolated in `C:\Users\Olga\AKB5-design` on `design/network-ui-polish`; keep `C:\Users\Olga\AKB5` / `Net` logic work separate until the user explicitly asks to merge or coordinate the branches.
- For Network passport consistency, dialog field validation should be mirrored at the mutation-service boundary when the same invalid value could otherwise be accepted through another caller.
