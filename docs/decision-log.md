...
- Network topology link-type selector decision: remove the long toolbar ComboBox and use a grouped floating canvas palette instead. Group the choices as `Profibus`, `MPI`, and `Profinet`, and distinguish optical/copper media with visible `опт.` / `медь` labels plus the real line color/dash sample. Clicking a link selects it, and palette clicks edit the selected link immediately; otherwise the palette sets the kind for the next new link.
