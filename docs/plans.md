...
- Current task: Network topology visible zoom-control follow-up is implemented locally after pushed commit `2c9d598`, with a Network-tab `ET200` -> `ET` visible-label adjustment, inline optional `Доп. IP` editing inside `Элемент сети`, a `Внешняя связь` text-card object/text-entry dialog mode for external systems, compact `docs/decision-log.md` cleanup, and repo-level shell guard. The user manually accepted the package and explicitly requested commit/push in the current chat.
