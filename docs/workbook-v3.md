...
- `LevelName` — legacy-совместимая подпись exchange-слоя

Статус:

- приложение больше не использует `LevelName` как пользовательскую настройку интерфейса
- лист `Levels` сохранён только как transition/legacy-слой совместимости workbook `v3`
- при необходимости можно править `LevelName` для обменного файла, но это не влияет на пользовательский UI приложения
...
- `Location` — редактируемая
- `PhotoPath` — редактируемая
- `IpAddress` — редактируемая для уровней `2+`, обычно пустая для верхних уровней
- `SchemaLink` — редактируемая для уровней `2+`, обычно пустая для верхних уровней
- `Path` — производная/display-only

Поддерживаемые ручные изменения:

- rename узла через `NodeName`
- правка карточки узла через `Description`, `Location`, `PhotoPath`, `IpAddress`, `SchemaLink`
...
- правки `LevelName` как замена реальному редактированию данных узла
- rename `WorkshopName`
- rename `NodeName`
- правки `Description`, `Location`, `PhotoPath`, `IpAddress`, `SchemaLink`
- stale `Meta.LastWorkshop`, если `LastWorkshopId` всё ещё валиден
- rename tab листа узлов
- перестановку колонок
- отсутствие новых detail-колонок в старом workbook `v3` — в этом случае поля узла считаются пустыми
...
Правильный сценарий редактирования: менять только бизнес-значения (`WorkshopName`, `NodeName`, `Description`, `Location`, `PhotoPath`, `IpAddress`, `SchemaLink`, выбор текущего цеха), а технические колонки не трогать. `Levels.LevelName` оставлен только для legacy-совместимости workbook `v3`.
